...
export async function getListArticleBySlug(slug: string) {
  try {
    const listArticle = await prisma.articles.findFirst({
      where: {
        slug,
        deleted_at: null,
      },
    ...
    if (!listArticle) return { error: "Article not found" }
...
export async function getRelatedArticles(articleId: string, categoryIds: string[], limit = 3) {
  try {
    // Get articles that share categories with the current article
    ...
    return { error: "Failed to fetch related articles" }
  }
}
...
export async function getArticleCategories() {
  try {
    ...
    return { categories: [], error: "Failed to load article categories" }
  }
}
