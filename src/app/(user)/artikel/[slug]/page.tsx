...
export default async function ArticleDetailPage({ params }: ArticleDetailPageProps) {
  ...
  try {
    ...
    const relatedArticlesData = await getRelatedArticles(article.id, categoryIds, 3)
    ...
    const categoriesWithCount = categories.map((category) => ({
      ...category,
      count: allArticles.filter((article) => article.categories?.some((cat) => cat.id === category.id)).length,
    }))

    // Get all unique tags from all articles
    const allTags = Array.from(
      new Set(allArticles.flatMap((article) => article.tag || []).map((tag) => JSON.stringify(tag))),
    ).map((tagStr) => JSON.parse(tagStr))

    // Prepare tags with count
    const tagsWithCount = allTags.map((tag) => ({
      ...tag,
      count: allArticles.filter((article) => article.tag?.some((t) => t.id === tag.id)).length,
    }))
