...
export function ListClassForm({ initialData, mentors, categories, tools, onSubmit, isSubmitting }: ListClassFormProps) {
  ...
  const toggleCategory = (categoryId: string) => {
    ...
    setValue('categories', selectedCategories.includes(categoryId) ? selectedCategories.filter((id) => id !== categoryId) : [...selectedCategories, categoryId]);
  };
  ...
  const toggleTool = (toolId: string) => {
    ...
    setValue('tools', selectedTools.includes(toolId) ? selectedTools.filter((id) => id !== toolId) : [...selectedTools, toolId]);
  };
    ...
    try {
      ...
      if (result.success) {
        Swal.fire({
          ...
          text: initialData ? 'List class has been updated successfully.' : 'New list class has been created successfully.',
        });
        ...
        Swal.fire({
          icon: 'error',
          title: 'Error',
          text: typeof result.error === 'string' ? result.error : 'Failed to save list class. Please check the form and try again.',
        });
      }
    } catch (error) {
      Swal.fire({
  ...
  const uploadImage = async (image: string) => {
    ...
    formData.append('key', '69121369a8a3072a634951909c896681');
  ...
  const onSubmitWithUpload = async (data: ListClassFormData) => {
    // Ensure categories, tools, and syllabus are included in the submission
    // const thumbnailURL =
    await uploadImage(data.thumbnail).catch((e) => {
      console.log(e);
      return { url: '' };
    });
    // const formData = {
    ...
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
      <div className="space-y-4">
        ...
        <div>
          ...
          <Input id="trailer" {...register('trailer')} className="mt-1" placeholder="https://www.youtube.com/watch?v=..." />
        ...
        <div>
          ...
          <Input id="meetings" type="number" min="1" {...register('meetings')} className="mt-1" placeholder="Enter number of meetings" />
        ...
        <div>
          ...
          <div className="mt-2 space-y-4">
            <div className="flex gap-2">
              <Input placeholder="Judul materi" value={newSyllabusTitle} onChange={(e) => setNewSyllabusTitle(e.target.value)} />
              <Button type="button" size="icon" onClick={addSyllabusItem} className="bg-blue-500 hover:bg-blue-600" disabled={!newSyllabusTitle.trim()}>
                    ...
                    <div className="flex justify-between items-center">
                      <div>
                        ...
                        <Input
                          value={item.title}
                          onChange={(e) => {
                            const updatedItems = syllabusItems.map((i) => (i.id === item.id ? { ...i, title: e.target.value } : i));
                            setSyllabusItems(updatedItems);
                            setValue('syllabus', updatedItems);
                          }}
                          className="mt-1"
                      ...
                      <div className="ml-4">
                        ...
                        <Input
                          ...
                          value={item.sort}
                          onChange={(e) => {
                            const updatedItems = syllabusItems.map((i) => (i.id === item.id ? { ...i, sort: Number.parseInt(e.target.value) || 1 } : i));
                            setSyllabusItems(updatedItems);
                            setValue('syllabus', updatedItems);
                          }}
                          className="mt-1 w-20"
                      ...
                      <Button type="button" variant="destructive" size="sm" onClick={() => removeSyllabusItem(item.id)} className="ml-4 self-end">
                        Hapus
                      </Button>
                    </div>
                  </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center space-x-2">
            <Checkbox id="is_popular" checked={watch('is_popular')} onCheckedChange={(checked) => setValue('is_popular', checked as boolean)} />
            <Label htmlFor="is_popular">Kelas Populer</Label>
            <input type="hidden" {...register('is_popular')} />
          </div>

          <div className="flex items-center space-x-2">
            <Checkbox id="is_request" checked={watch('is_request') || false} onCheckedChange={(checked) => setValue('is_request', checked as boolean | null)} />
            <Label htmlFor="is_request">Kelas Request</Label>
            <input type="hidden" {...register('is_request')} />
          </div>

          <div className="flex items-center space-x-2">
            <Checkbox id="is_active" checked={watch('is_active')} onCheckedChange={(checked) => setValue('is_active', checked as boolean)} />
