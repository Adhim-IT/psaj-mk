  ...
  useEffect(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter(
        (article) =>
          article.title.toLowerCase().includes(query) || extractExcerpt(article.content).toLowerCase().includes(query),
      )
    }
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center text-xs text-gray-500">
                      {article.categories && article.categories[0] && (
                        <span
                          className="mr-3 cursor-pointer hover:text-blue-600"
                          onClick={() => setSelectedCategory(article.categories![0].id)}
                        >
          ...
          <div className="space-y-2">
            {categories.map((category) => (
              ...
              >
                <span className="flex items-center">
                  <ChevronRight
                    className={`h-4 w-4 mr-1 ${selectedCategory === category.id ? "text-blue-600" : "text-gray-400"}`}
                  />
                  {category.name}
                </span>
          ...
          <div className="flex flex-wrap gap-2">
            {tags.map((tag) => (
              <span
                key={tag.id}
                onClick={() => setSelectedTag(selectedTag === tag.id ? null : tag.id)}
                className={`inline-flex items-center gap-1 px-3 py-1.5 rounded-md text-sm cursor-pointer transition-colors ${
                  selectedTag === tag.id ? "bg-[#5596DF] text-white" : "bg-gray-100 text-[#5596DF] hover:bg-gray-200"
                }`}
                style={{
              ...
              >
                ...
                <span
                  className={`px-1.5 py-0.5 rounded-full text-xs ml-1 ${
                    selectedTag === tag.id ? "bg-white/20 text-white" : "bg-gray-200 text-gray-700"
                  }`}
                >
                  {tag.count}
                </span>
