      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          <div className="relative w-full h-64 md:h-96 rounded-2xl overflow-hidden mb-6">
            <Image src={typeof event.thumbnail === 'string' && event.thumbnail ? event.thumbnail : '/placeholder.svg'} alt={event.title} fill className="object-cover" onError={() => setImageError(true)} unoptimized={true} />
          </div>
            ...
            <div className="bg-gray-50 p-6 rounded-xl mb-8">
              ...
              <div className="flex items-center">
                <Avatar className="h-16 w-16 mr-4">
                  <AvatarImage
                    src={typeof event.mentors.profile_picture === 'string' && event.mentors.profile_picture ? event.mentors.profile_picture : 'https://res.cloudinary.com/dtrfxupze/image/upload/v1742300883/Group_33_2_xbwbi0.png'}
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-xl shadow-md p-6 sticky top-24">
            ...
            <Button
              ...
              className={`w-full ${registrationStatus === 'paid' ? 'bg-green-500 hover:bg-green-600' : 'bg-[#4A90E2] hover:bg-[#3178c6]'} ${registrationStatus === 'pending' ? 'opacity-70' : ''} ${
            ...
            {registrationStatus === 'rejected' && <p className="text-red-500 text-sm mt-2 text-center">Pembayaran Anda ditolak. Silakan hubungi admin untuk informasi lebih lanjut.</p>}

            {registrationStatus === 'pending' && <p className="text-amber-500 text-sm mt-2 text-center">Menunggu konfirmasi pembayaran dari admin.</p>}
          </div>
        </div>
      </div>

      {/* Registration Modal */}
      <EventRegistrationModal event={event} isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSubmit={handleSubmitRegistration} />
    </div>
  );
}
