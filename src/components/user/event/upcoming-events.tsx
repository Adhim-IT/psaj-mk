    ...
    <section className="py-16 bg-gray-50">
      <div className="container mx-auto px-6 xl:px-8 max-w-7xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {events.map((event) => (
            <div key={event.id} className="bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300 flex flex-col">
              <div className="relative h-48">
                <Image
                  ...
                  unoptimized={true}
                />
                {!event.price && <div className="absolute top-3 right-3 bg-green-500 px-3 py-1 rounded-full text-xs font-medium text-white">Gratis</div>}
                ...
                <div className="space-y-2 mb-4">
                  ...
                  {event.mentors && (
                    <div className="flex items-center text-sm text-gray-500">
                      ...
                      {event.mentors.specialization && <span className="ml-1 text-gray-400">• {event.mentors.specialization}</span>}
                    </div>
                  )}
                  <div className="flex items-center text-sm text-gray-500">
              ...
              <div className="p-6 pt-0 mt-auto">
                {event.price ? <p className="text-[#4A90E2] font-bold text-lg mb-3">Rp {formatPrice(event.price)}</p> : <p className="text-green-500 font-bold text-lg mb-3">Gratis</p>}
        ...
        <div className="mt-12 text-center">
          <Link href="/event" className="inline-flex items-center text-[#4A90E2] font-medium hover:text-[#3178c6] transition-colors">
            Lihat Semua Event
