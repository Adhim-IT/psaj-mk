...
export default function DetailCourse({ course, mentor, courseTypes = [] }: DetailCourseProps) {
  ...
  const mentorImage = mentor?.profile_picture || "/images/mentor.png"
              ...
              <div className="flex flex-wrap gap-2">
                {course.categories.map((category) => (
                  <Badge
                    key={category.id}
                    variant="secondary"
                    className="text-xs font-medium bg-white/20 hover:bg-white/30 transition-colors duration-200"
                  >
                    {category.name}
                  </Badge>
                ))}
            ...
            <div className="flex flex-wrap items-center gap-4 mt-2">
              <div className="flex items-center gap-2 bg-white/10 rounded-full px-3 py-1.5 backdrop-blur-sm">
                <Avatar className="h-8 w-8 border-2 border-white/30">
                  <AvatarImage src={mentorImage} alt={mentor?.name || "Mentor"} />
              ...
              <div className="flex items-center">
                <Badge
                  variant="outline"
                  className="text-xs font-medium border-white/30 text-primary-foreground bg-white/10 rounded-full px-3 py-1 backdrop-blur-sm"
                >
                    ...
                    <div className="flex flex-wrap gap-2">
                      {course.categories.map((category) => (
                        <Badge
                          key={category.id}
                          variant="outline"
                          className="text-sm bg-gray-50 hover:bg-gray-100 transition-colors duration-200"
                        >
                          {category.name}
                        </Badge>
                      ))}
                  ...
                  <div className="space-y-4">
                    {course.syllabus && course.syllabus.length > 0 ? (
                      course.syllabus.map((item, index) => (
                        <div
                          key={item.id}
                          className="flex gap-4 items-start p-4 rounded-xl bg-blue-50/50 hover:bg-blue-50 transition-colors duration-200"
                        >
                          <div className="flex items-center justify-center w-8 h-8 rounded-full bg-[#5596DF] text-white font-bold text-sm">
                            {index + 1}
                          </div>
                          <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                    {course.tools && course.tools.length > 0 ? (
                      course.tools.map((tool) => (
                        <div
                          key={tool.id}
                          className="bg-blue-50/50 rounded-xl p-5 hover:shadow-md transition-all duration-200 border border-transparent hover:border-blue-100"
                        >
                          <div className="mb-3">
                            ...
                            {"logo" in tool && tool.logo ? (
                              ...
                              <div className="w-20 h-20 bg-green-300 rounded-xl flex items-center justify-center text-gray-800 font-bold">
                                {tool.name.charAt(0)}
                              </div>
                            )}
                          </div>
                          ...
                          {"url" in tool && tool.url && (
                            <a href={tool.url} target="_blank" rel="noopener noreferrer">
                              <Button
                                variant="default"
                                className="bg-[#5596DF] hover:bg-[#4a89dc] text-white rounded-full px-6 transition-colors duration-200"
                              >
                                Download
                              </Button>
                            </a>
              ...
              <Card className="overflow-hidden rounded-xl shadow-lg border-0 transition-all duration-300 hover:shadow-xl">
                <div className="relative">
                  ...
                  <div className="absolute bottom-3 left-3 right-3 flex justify-between items-center">
                    <Badge className="bg-[#5596DF]/90 hover:bg-[#5596DF] transition-colors duration-200">
                      {course.level || "Beginner"}
                    </Badge>
                    <Badge
                      variant="outline"
                      className="bg-black/50 text-white border-white/20 hover:bg-black/60 transition-colors duration-200"
                    >
                  ...
                  <div className="space-y-5">
                    <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                      <Avatar className="h-12 w-12 border border-[#5596DF]/20">
                        <AvatarImage src={mentorImage} alt={mentor?.name || "Mentor"} />
                        <AvatarFallback className="bg-[#5596DF]/10 text-[#5596DF]">
                          {mentor?.name?.charAt(0) || "M"}
                        </AvatarFallback>
                      </Avatar>
                      <div>
