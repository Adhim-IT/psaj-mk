...
// Define the mentor type to match your database schema
type Mentor = {
  id: string
  name: string
...
export default function MentorSection({ mentors, maxMentors }: MentorSectionProps) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      {displayedMentors.map((mentor) => (
        <Link href={`/mentor/${mentor.name}`} key={mentor.id} className="block">
